...
This document is for Openshift version 4.16 and CoreOS 4.16. If you are installing Openshift version 4.15 please refer to ocp415 branch

## Step 1 - acc-provision
* Provision ACI fabric using acc-provision utility. 

  * Specify the flavor parameter value as 'openshift-4.16-esx'.
  * Specify an archive tar file for '-z' option, the archive file created will be required in the next steps
  
  Example 
  `acc-provision -a -c acc_provision_input.yaml -u admin -p ### -f openshift-4.16-esx  -z manifests.tar.gz`
...
* Download OCP416 OVA from Redhat site and import it. Specify **api-vlan-portgroup** as the port group for network interface.
